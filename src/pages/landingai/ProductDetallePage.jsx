...
const ProductoDetallePage = () => {
  ...
  const step1Done = !!selectedTemplate;
  const step2Done = userImages.length > 0;
  const step3Done = selectedEtapas.length > 0;
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
      ...
      {generating && (
        ...
        >
          <div className="flex flex-col items-center justify-center gap-7 max-w-sm px-6">
            {/* Orb */}
            <div style={{ position: "relative", width: 100, height: 100 }}>
              ...
              <div
                style={{
                  ...
                  inset: 0,
                  borderRadius: "50%",
                  background: "linear-gradient(135deg,#6366f1,#38bdf8,#67e8f9)",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  boxShadow:
                    "0 0 50px rgba(56,189,248,.4),0 0 100px rgba(99,102,241,.15)",
            ...
            <div className="text-center">
              <p className="text-xl font-black text-gray-900 mb-1">
                Generando sección {genProgress.current}/{genProgress.total}
              </p>
              <p className="text-sm font-semibold" style={{ color: "#6366f1" }}>
                {genProgress.etapa}
              </p>
              <p className="text-[11px] text-gray-400 mt-3">
                ~30 segundos por sección · No cierres la página
              </p>
            </div>
                    ...
                    style={{
                      background: done
                        ...
                        : active
                          ? "rgba(99,102,241,.08)"
                          : "#f1f5f9",
                      color: done ? "#059669" : active ? "#6366f1" : "#9ca3af",
                      border: `1.5px solid ${done ? "rgba(16,185,129,.2)" : active ? "rgba(99,102,241,.25)" : "#e2e8f0"}`,
                      animation: active
                        ? "gPill 1.5s ease-in-out infinite"
                        : "none",
                    }}
                  >
                    {done ? "✓ " : active ? "⟳ " : ""}
      ...
      {genOpen && !generating && (
        ...
        >
          <div className="p-5 sm:p-6">
            ...
            >
              <Chip n={1} done={step1Done} label="Template" />
              <Bar done={step1Done} />
              <Chip n={2} done={step2Done} label="Fotos" />
              <Bar done={step2Done} />
              <Chip n={3} done={step3Done} label="Secciones" />
              <div
                className="ml-auto hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg"
                style={{
                ...
                {displayTemplates.map((t) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {sel && (
                        <div
                          className="absolute top-1 right-1 w-5 h-5 rounded-md grid place-items-center"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              ...
              >
                <span className="text-[11px] font-bold tracking-wide block mb-3 text-gray-500">
                  FOTOS{" "}
                  ...
                  {userImages.map((img, i) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 w-5 h-5 rounded-md grid place-items-center opacity-0 group-hover:opacity-100 transition"
                        style={{ background: "rgba(239,68,68,.9)" }}
                      >
                        <i className="bx bx-x text-white text-xs" />
                      </button>
                    </div>
                  ))}
                  {userImages.length < 6 && (
                    <button
                      ...
                      className="rounded-xl grid place-items-center transition hover:border-indigo-400"
                      style={{
                        aspectRatio: "1/1",
                    ...
                    >
                      <div className="text-center">
                        <i className="bx bx-image-add text-lg text-gray-300" />
                        <p className="text-[9px] mt-0.5 text-gray-300">Subir</p>
                      </div>
                    </button>
                  )}
                </div>
                  ...
                  {etapas.map((e) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          background: sel ? "rgba(99,102,241,.08)" : "white",
                          border: `1px solid ${sel ? "rgba(99,102,241,.3)" : "#e2e8f0"}`,
                          color: sel ? "#6366f1" : "#64748b",
                        }}
                      >
                        {sel && <i className="bx bx-check text-xs mr-1" />}
                        {e.nombre}
                      </button>
                    );
                  })}
            ...
            >
              <span className="text-[11px] hidden sm:block text-gray-400">
                {selectedTemplate ? "✓ Template" : "— Template"} ·{" "}
                {userImages.length > 0
                  ? `✓ ${userImages.length} foto${userImages.length > 1 ? "s" : ""}`
                  : "— Fotos"}{" "}
                ·{" "}
              ...
              >
                <i className="bx bx-bolt-circle text-base" /> Generar{" "}
                {selectedEtapas.length > 0
                  ? `${selectedEtapas.length} sección${selectedEtapas.length !== 1 ? "es" : ""}`
                  : ""}
              </button>
            ...
            >
              ...
              <p className="text-[11px] flex-1 text-gray-500">
                ¿Ángulos de venta o más opciones?{" "}
                <button
                  onClick={goToFullGenerator}
      ...
      <div className="pt-5">
        {generaciones.length === 0 && !loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6">
            ...
            {/* Sidebar — CLEAN, no repeated buttons */}
              ...
              <div className="sticky top-6 flex flex-col gap-4 max-h-[calc(100vh-6rem)] overflow-y-auto">
                ...
                >
                  ...
                  <div className="space-y-2">
                    <InfoRow label="Nombre" value={producto.nombre} />
                    {producto.marca && (
                    ...
                    {producto.precio_unitario && (
                      <InfoRow
                        label="Precio"
                        value={`${producto.moneda} ${producto.precio_unitario}`}
                      />
                    )}
                    <InfoRow label="Imágenes" value={pagination.total} />
                    <InfoRow
                      label="Creado"
...
const MiniReveal3D = ({ item, phase }) => {
  ...
  return (
    ...
    >
      <div className={`absolute mR-burst mR-burst-${phase}`} />
      ...
      <style>{`
        ...
        .mR-label{transition:all .6s cubic-bezier(.34,1.56,.64,1)}.mR-label-enter{opacity:0;transform:translateY(30px) scale(.8)}.mR-label-showcase{opacity:1;transform:translateY(0) scale(1);animation:mrLF 2s ease-in-out infinite}.mR-label-exit{opacity:0;transform:translateY(-20px) scale(.9)}@keyframes mrLF{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}}
        ...
        @keyframes mrC{0%{transform:scale(.1) rotateY(180deg) rotateX(30deg);opacity:0;filter:brightness(3) saturate(0)}15%{transform:scale(.6) rotateY(120deg) rotateX(15deg);opacity:1;filter:brightness(2) saturate(.5)}35%{transform:scale(1.1) rotateY(45deg) rotateX(-5deg);filter:brightness(1.5) saturate(.8)}50%{transform:scale(1.05) rotateY(-10deg) rotateX(3deg)}65%{transform:scale(1.02) rotateY(5deg) rotateX(-1deg)}80%{transform:scale(1) rotateY(-2deg)}100%{transform:scale(1) rotateY(0);filter:brightness(1) saturate(1)}}
        @keyframes mrCE{0%{transform:scale(1);opacity:1}100%{transform:scale(.3) rotateY(-30deg) translateY(40px);opacity:0}}
        .mR-glow{transition:all .6s}.mR-glow-enter{opacity:0}.mR-glow-showcase{opacity:1;box-shadow:0 0 60px rgba(99,102,241,.4),0 0 120px rgba(99,102,241,.2);animation:mrGP 2s ease-in-out infinite}.mR-glow-exit{opacity:0}@keyframes mrGP{0%,100%{box-shadow:0 0 60px rgba(99,102,241,.4),0 0 120px rgba(99,102,241,.2)}50%{box-shadow:0 0 80px rgba(99,102,241,.5),0 0 160px rgba(99,102,241,.3)}}
        .mR-shine-enter{opacity:0}.mR-shine-showcase::after{content:'';position:absolute;top:0;left:-80%;width:50%;height:100%;background:linear-gradient(90deg,transparent,rgba(255,255,255,.35),transparent);transform:skewX(-15deg);animation:mrSh 1.2s ease-in-out 1.2s forwards}.mR-shine-exit{opacity:0}@keyframes mrSh{from{left:-50%}to{left:130%}}
      `}</style>
    </div>
  );
};
...
/* ═══ Micro components ═══ */
const Chip = ({ n, done, label }) => (
  <div className="flex items-center gap-1.5">
    <div
      className="w-7 h-7 rounded-lg grid place-items-center text-[10px] font-bold transition-all"
      style={{
        background: done ? "rgba(99,102,241,.1)" : "#f1f5f9",
        border: `1.5px solid ${done ? "rgba(99,102,241,.3)" : "#e2e8f0"}`,
        color: done ? "#6366f1" : "#94a3b8",
      }}
    >
      {done ? <i className="bx bx-check text-xs" /> : n}
    </div>
    <span
      className="text-[10px] font-semibold hidden sm:inline"
      style={{ color: done ? "#6366f1" : "#94a3b8" }}
    >
      {label}
    </span>
  </div>
...
const Bar = ({ done }) => (
  <div
    ...
    style={{ background: done ? "rgba(99,102,241,.3)" : "#e2e8f0" }}
  />
);
